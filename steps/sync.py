...
def step_sync() -> None:
    print(f"\n{color.header('▶ STEP 4: SYNC TAGS → CATEGORIES')}\n")
    if is_dry_run():
        dry_run_banner()

    all_categories = get_all("/api/organizers/categories")
    cat_by_norm    = {normalize(c["name"]): c for c in all_categories}
    print(f"  {color.bold('Categories:')} {color.muted(str(sorted(c['name'] for c in all_categories)))}\n")
    ...
    for recipe in sorted(recipes, key=lambda r: r["name"].lower()):
        ...
        try:
            ...
            print(f"  {color.ok('✓')} {color.bold(title)} {color.muted('→ added:')} {color.bright_cyan(str(adding))}")
            updated += 1
    ...
    print(f"\n{color.ok('✓')} {color.bold('Sync complete.')}")
    print(f"  Updated : {color.ok(str(updated))}")
    print(f"  Skipped : {color.muted(str(skipped))}  (already in sync)")
